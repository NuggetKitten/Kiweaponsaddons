...
namespace kiweaponsaddons.Projectiles
{
    public class thinbem : ModProjectile
    {
        // A helpful math constant for performing beam angling calculations.
        private const float PiBeamDivisor = MathHelper.Pi / thinlaz.NumBeams;
        ...
        private const float MaxBeamSpread = 3f;
        ...
        private const float BeamHueVariance = 0.08f;
        ...
        public override void AI()
        {
            ...
            if (projectile.type != ModContent.ProjectileType<thinbem>() || !hostPrism.active || hostPrism.type != ModContent.ProjectileType<thinlaz>())
            {
                projectile.Kill();
                return;
            }
            ...
            float chargeRatio = MathHelper.Clamp(hostPrism.ai[0] / thinlaz.MaxCharge, 0f, 1f);
            ...
            projectile.friendly = hostPrism.ai[0] > thinlaz.DamageStart;

            // This offset is used to make each individual beam orient differently based on its Beam ID.
            float beamIdOffset = BeamID - thinlaz.NumBeams / 2f + 0.5f;
            float beamSpread;
            float spinRate;
            ...
            float deviationAngle = (hostPrism.ai[0] + beamIdOffset / spinRate) / (spinRate * thinlaz.NumBeams) * MathHelper.TwoPi;
        ...
        private Color GetOuterBeamColor()
        {
            ...
            float hue = (BeamID / thinlaz.NumBeams) % BeamHueVariance + BeamColorHue;
